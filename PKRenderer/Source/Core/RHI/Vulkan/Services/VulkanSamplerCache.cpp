...
namespace PK
{
    ...
    VulkanSampler* VulkanSamplerCache::GetPooledSampler(const SamplerDescriptor& descriptor)
    {
        ...
        {
            m_samplers[index].value = m_samplerPool.New(m_device, descriptor, FixedString64("Sampler%u", index).c_str());
            ...
            bindHandle->image.sampler = m_samplers[index].value->sampler;
