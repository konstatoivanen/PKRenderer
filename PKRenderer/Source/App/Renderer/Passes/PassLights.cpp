...
namespace PK::App
{
    ...
    void PassLights::BuildShadowCascadeMatrices(const ShadowCascadeCreateInfo info, float4x4* outMatrices)
    {
        ...
        AABB<float3>* aabbs = PK_STACK_ALLOC(AABB<float3>, info.count);
        ...
        for (auto i = 0u; i < info.count; ++i)
        {
            ...
            outMatrices[i] = math::orthographic(aabbs[i].min.x, aabbs[i].max.x, aabbs[i].min.y, aabbs[i].max.y, znear, zfar) * info.worldToLocal;
        }
    }
}
