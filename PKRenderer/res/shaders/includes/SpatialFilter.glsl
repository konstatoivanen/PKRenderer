...

{
    ...
    float k_V = 1.0f / (0.05f * SFLT_DEPTH);
    float k_H = 1.0f / (2.0f * SFLT_RADIUS * SFLT_RADIUS);
    
    // Normal
    float k_N = PK_HALF_PI * pow2(SFLT_ROUGHNESS) / (1.0f + pow2(SFLT_ROUGHNESS));
    k_N *= lerp(0.5f, 1.0f, 1.0f / (SFLT_HISTORY + 1.0f));
    k_N = 1.0f / max(k_N, 1e-4f);
    ...
    {
        const float3 s_o = PK_POISSON_DISK_32_POW[i];
        const float3 s_v = SFLT_VPOS + basis * rotate2D(s_o.xy, rotation);
        const float3 s_c = mul(pk_MATRIX_P, float4(s_v, 1.0f)).xyw;
        const float2 s_uv = (s_c.xy / s_c.z) * 0.5f + 0.5f;
        ...
        const float w_n = exp(-acos(dot(SFLT_NORMAL, s_nr.xyz)) * k_N);
            ...
            const float w = w_h * w_v * w_n * w_r * s_o.z;
        #else
            const float w = w_h * w_v * w_n * s_o.z;
        #endif
