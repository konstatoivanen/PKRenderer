...
struct DispatchParameters
{
    ...
    //#define BEND_SAMPLE_DEPTH(uv)
    //#define BEND_SAMPLE_DEPTH_OFFSET(uv, offset)
...
void ComputeWavefrontExtents(DispatchParameters inParameters, int3 inGroupID, uint inGroupThreadID, out float2 outDeltaXY, out float2 outPixelXY, out float outPixelDistance, out bool outMajorAxisX)
{
...
void WriteScreenSpaceShadow(DispatchParameters inParameters, int3 inGroupID, int inGroupThreadID)
{
    ...
    [[unroll]] for (i = 0; i < READ_COUNT; i++)
    {
        ...
        depths.x = BEND_SAMPLE_DEPTH(read_xy * inParameters.InvDepthTextureSize);
        depths.y = BEND_SAMPLE_DEPTH((read_xy + offset_xy) * inParameters.InvDepthTextureSize);
    #else
       // int bias = bilinear > 0 ? 1 : -1;
       // int2 offset_xy = int2(x_axis_major ? 0 : bias, x_axis_major ? bias : 0);
       //
       // depths.x = BEND_SAMPLE_DEPTH(read_xy * inParameters.InvDepthTextureSize);
       // depths.y = BEND_SAMPLE_DEPTH_OFFSET(read_xy * inParameters.InvDepthTextureSize, offset_xy);
    #endif
    ...
    if (inParameters.UseEarlyOut && (inParameters.DebugOutputWaveIndex == false && inParameters.DebugOutputThreadIndex == false && inParameters.DebugOutputEdgeMask == false))
    {
        ...
        bool early_out = subgroupAny(!skip_pixel) == false;
    ...
    result = dot(shadow_value, 0.25f.xxxx);
    ...
    //write the result
    {
        ...
        if (inParameters.DebugOutputWaveIndex)			
            result = fract(inGroupID.x / float(WAVE_SIZE));
