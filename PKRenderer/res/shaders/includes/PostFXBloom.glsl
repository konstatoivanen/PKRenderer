...
float3 Bloom(float3 color, float2 uv)
{
	...
	bloomLens += tex2DLod(pk_Bloom_Texture, uv + (float2(0, 0) - 0.5f.xx) * tx * 4.0f, 3).rgb;
	bloomLens += tex2DLod(pk_Bloom_Texture, uv + (float2(0, 1) - 0.5f.xx) * tx * 4.0f, 3).rgb;
	bloomLens += tex2DLod(pk_Bloom_Texture, uv + (float2(1, 1) - 0.5f.xx) * tx * 4.0f, 3).rgb;
	bloomLens += tex2DLod(pk_Bloom_Texture, uv + (float2(1, 0) - 0.5f.xx) * tx * 4.0f, 3).rgb;
	bloomLens *= 0.25f;

	const float lensdirt = tex2D(pk_Bloom_LensDirtTex, uv).r;

	color = lerp(color, bloom, pk_Bloom_Intensity.xxx);
	color = lerp(color, bloomLens * 0.333f, lensdirt * pk_Bloom_DirtIntensity.xxx);
