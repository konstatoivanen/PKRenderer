...
namespace PK::Rendering::RHI::Vulkan::Objects
{
    ...
    void VulkanSwapchain::Rebuild(const SwapchainCreateInfo& createInfo)
    {
        ...
        for (size_t i = 0u; i < m_imageCount; ++i)
        {
            ...
            m_imageViews[i] = RHI::Driver::GetNative<VulkanDriver>()->imageViewPool.New(m_device, imageViewCreateInfo, (std::string("Swapchain.Image") + std::to_string(i)).c_str());
        }
    ...
    void VulkanSwapchain::Release()
    {
        for (size_t i = 0u; i < MaxImageCount; ++i)
        {
            ...
            {
                RHI::Driver::GetNative<VulkanDriver>()->imageViewPool.Delete(m_imageViews[i]);
