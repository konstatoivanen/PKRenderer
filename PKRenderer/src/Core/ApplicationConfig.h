...
namespace PK::Core
{
    PK_YAML_ASSET_BEGIN(ApplicationConfig, ".cfg")
        ...
        PK_YAML_MEMBER(std::string, FileWindowIcon, "res/T_AppIcon.bmp")

        PK_YAML_MEMBER(Math::uint, RandomSeed, 512u)

        PK_YAML_MEMBER(Math::float3, CameraStartPosition, Math::PK_FLOAT3_ZERO)
        PK_YAML_MEMBER(Math::float3, CameraStartRotation, Math::PK_FLOAT3_ZERO)
        ...
        PK_YAML_MEMBER(Math::uint, LightCount, 0u)
        PK_YAML_MEMBER(Math::uint, ShadowmapTileSize, 512u)
        ...
        PK_YAML_MEMBER(Math::uint, CC_Shadows, 0x000000FF)
        PK_YAML_MEMBER(Math::uint, CC_Midtones, 0x7F7F7FFF)
        PK_YAML_MEMBER(Math::uint, CC_Highlights, 0xFFFFFFFF)
        PK_YAML_MEMBER(Math::uint, CC_ChannelMixerRed, 0xFF0000FF)
        PK_YAML_MEMBER(Math::uint, CC_ChannelMixerGreen, 0x00FF00FF)
        PK_YAML_MEMBER(Math::uint, CC_ChannelMixerBlue, 0x0000FFFF)
        ...
        PK_YAML_MEMBER(Math::float3, FogAlbedo, Math::PK_FLOAT3_ONE)
        PK_YAML_MEMBER(Math::float3, FogAbsorption, Math::PK_FLOAT3_ONE)
        PK_YAML_MEMBER(float, FogPhase0, 0.25f)
        PK_YAML_MEMBER(float, FogPhase1, 0.95f)
        PK_YAML_MEMBER(float, FogPhaseW, 0.5f)
        PK_YAML_MEMBER(Math::float3, FogWindDirection, Math::PK_FLOAT3_FORWARD)
