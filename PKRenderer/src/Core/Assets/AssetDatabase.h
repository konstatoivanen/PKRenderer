...
{
    class AssetDatabase : public Services::IService
    {
        public:
            ...
            template<typename T, typename ... Args>
            T* Load(const std::string& filepath, Args&& ... args) { return LoadInternal<T>(Asset::NameToId(filepath), false, std::forward<Args>(args)...); }

            template<typename T, typename ... Args>
            T* Reload(const std::string& filepath, Args&& ... args) { return LoadInternal<T>(Asset::NameToId(filepath), true, std::forward<Args>(args)...); }
            ...
            {
                static_assert(std::is_base_of<Asset, T>::value, "Template argument type does not derive from Asset!");
                Unload<T>(Asset::NameToId(filepath));
            }
        ...
        private:
            ...
            [[nodiscard]] T* LoadInternal(AssetID assetId, bool isReload, Args&& ... args)
            {
                ...
                auto filepath = Asset::IdToName(assetId);
            ...
            [[nodiscard]] T* RegisterInternal(const char* name, Utilities::Ref<T> asset)
            {
                ...
                auto assetId = Asset::NameToId(name);
