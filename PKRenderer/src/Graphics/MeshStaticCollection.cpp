...
namespace PK::Graphics
{
    ...
    MeshStaticCollection::MeshStaticCollection()
    {
        ...
        m_positionsBuffer = RHICreateBuffer(m_streamLayout.GetStride(1u) * 2000000, BufferUsage::SparseVertex | BufferUsage::Storage, "MeshStaticCollection.VertexPositions");
        m_attributesBuffer = RHICreateBuffer(m_streamLayout.GetStride(0u) * 2000000, BufferUsage::SparseVertex, "MeshStaticCollection.VertexAttributes");
        m_indexBuffer = RHICreateBuffer(ElementConvert::Size(m_indexType) * 2000000, BufferUsage::SparseIndex | BufferUsage::Storage, "MeshStaticCollection.IndexBuffer");
    ...
    Buffer* MeshStaticCollection::GetPositionBuffer() const { return m_positionsBuffer.get(); }
    Buffer* MeshStaticCollection::GetAttributeBuffer() const { return m_attributesBuffer.get(); }
    Buffer* MeshStaticCollection::GetIndexBuffer() const { return m_indexBuffer.get(); }
    Buffer* MeshStaticCollection::GetMeshletVertexBuffer() const { return m_meshletVertexBuffer.get(); }
    Buffer* MeshStaticCollection::GetMeshletIndexBuffer() const { return m_meshletIndexBuffer.get(); }
    Buffer* MeshStaticCollection::GetMeshletSubmeshBuffer() const { return m_submeshBuffer.get(); }
    Buffer* MeshStaticCollection::GetMeshletBuffer() const { return m_meshletBuffer.get(); }
    ...
    MeshStatic* MeshStaticCollection::Allocate(MeshStaticAllocationData* data)
    {
        ...
        AlignVertexStreams((char*)data->regular.pVertices, data->regular.vertexCount, data->regular.streamLayout, m_streamLayout);
    ...
    bool MeshStaticCollection::TryGetAccelerationStructureGeometryInfo(uint32_t globalSubmeshIndex, RHI::AccelerationStructureGeometryInfo* outInfo) const
    {
        if (!HasPendingUpload())
        {
    ...
    bool MeshStatic::TryGetAccelerationStructureGeometryInfo(uint32_t localIndex, RHI::AccelerationStructureGeometryInfo* outInfo) const
    {
