...
namespace PK::Graphics::RHI::Vulkan::Objects
{
    ...
    VulkanTexture::VulkanTexture(const TextureDescriptor& descriptor, const char* name) : RHITexture(name), m_driver(RHIDriver::GetNative<VulkanDriver>())
    {
        Rebuild(descriptor);
    }
    ...
    void VulkanTexture::SetSampler(const SamplerDescriptor& sampler)
    {
        ...
        for (auto i = 0u; i < m_imageViews.GetCount(); ++i)
        {
            ...
            if (value->bindHandle->image.sampler != VK_NULL_HANDLE)
            {
                ...
                value->bindHandle->image.sampler = RHIDriver::GetNative<VulkanDriver>()->samplerCache->GetSampler(m_descriptor.sampler);
            }
        }
    }
    ...
    void VulkanTexture::FillBindHandle(VulkanBindHandle* handle, const TextureViewRange& range, TextureBindMode bindMode) const
    {
        ...
        {
            handle->image.sampler = RHIDriver::GetNative<VulkanDriver>()->samplerCache->GetSampler(m_descriptor.sampler);
        }
    }
    ...
    const VulkanTexture::ViewValue* VulkanTexture::GetView(const TextureViewRange& range, TextureBindMode mode)
    {
        ...
        {
            viewValue->bindHandle->image.sampler = RHIDriver::GetNative<VulkanDriver>()->samplerCache->GetSampler(m_descriptor.sampler);
        }
