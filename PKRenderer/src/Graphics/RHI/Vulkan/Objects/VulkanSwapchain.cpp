...
namespace PK::Graphics::RHI::Vulkan::Objects
{
    ...
    void VulkanSwapchain::Rebuild(const SwapchainCreateInfo& createInfo)
    {
        ...
        for (size_t i = 0u; i < m_imageCount; ++i)
        {
            ...
            m_imageViews[i] = RHIDriver::GetNative<VulkanDriver>()->imageViewPool.New(m_device, imageViewCreateInfo, (std::string("Swapchain.Image") + std::to_string(i)).c_str());
        }
    ...
    void VulkanSwapchain::Release()
    {
        for (size_t i = 0u; i < MaxImageCount; ++i)
        {
            ...
            {
                RHIDriver::GetNative<VulkanDriver>()->imageViewPool.Delete(m_imageViews[i]);
