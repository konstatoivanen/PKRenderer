...
namespace PK::Graphics::RHI::Vulkan::Objects
{
    ...
    VulkanShader::VulkanShader(void* base, PK::Assets::Shader::PKShaderVariant* variant, const char* name) :
        m_device(RHIDriver::GetNative<VulkanDriver>()->device),
        m_name(name)
    {
        ...
        auto layoutCache = RHIDriver::GetNative<VulkanDriver>()->layoutCache.get();
    ...
    {
        auto driver = RHIDriver::GetNative<VulkanDriver>();
