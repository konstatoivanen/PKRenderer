...
namespace PK::Graphics::RHI::Vulkan::Services
{
    ...
    void VulkanStagingBufferCache::Release(VulkanStagingBuffer* buffer, const FenceRef& fence)
    {
        ...
        if (buffer->persistentmap)
        {
            auto deleter = [](void* v)
            {
                RHIDriver::GetNative<VulkanDriver>()->stagingBufferCache->m_bufferPool.Delete(reinterpret_cast<VulkanStagingBuffer*>(v));
            };

            RHIDriver::GetNative<VulkanDriver>()->disposer->Dispose(buffer, deleter, fence);
        }
        else
        {
