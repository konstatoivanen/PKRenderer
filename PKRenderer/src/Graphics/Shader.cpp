...
namespace PK::Graphics
{
    ...
    bool Shader::Map::SupportsKeywords(const NameID* names, const uint32_t count) const
    {
    ...
    uint32_t Shader::Map::GetIndex(const NameID* names, size_t count) const
    {
    ...
    void Shader::Map::Selector::SetKeywordsFrom(const PropertyBlock& block)
    {
        bool value;
    ...
    void Shader::AssetImport(const char* filepath)
    {
        ...
        for (auto i = 0u; i < shader->variantcount; ++i)
        {
            ...
            m_shaders.push_back(RHICreateShader(base, pVariants + i, name.c_str()));
        }
...
template<>
bool PK::Core::Assets::Asset::IsValidExtension<PK::Graphics::Shader>(const std::string& extension) { return extension.compare(".pkshader") == 0; }

template<>
PK::Utilities::Ref<PK::Graphics::Shader> PK::Core::Assets::Asset::Create() { return PK::Utilities::CreateRef<PK::Graphics::Shader>(); }
