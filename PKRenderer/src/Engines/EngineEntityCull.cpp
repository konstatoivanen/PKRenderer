...
namespace PK::Engines
{
    ...
    void EngineEntityCull::Step(RequestEntityCullFrustum* request)
    {
        ...
        auto cullingPlanes = Functions::ExtractFrustrumPlanes(request->matrix, true);
        ...
        for (auto i = 0u; i < entityViews.count; ++i)
        {
            ...
            {
                if ((viewFlags & ScenePrimitiveFlags::NeverCull) != 0 || Functions::IntersectPlanesAABB(cullingPlanes.array_ptr(), 6, entityView->bounds->worldAABB))
                {
                    auto depth = Functions::PlaneMaxDistanceToAABB(cullingPlanes.near, entityView->bounds->worldAABB);
    ...
    void EngineEntityCull::Step(RequestEntityCullCubeFaces* request)
    {
        ...
        for (auto i = 0u; i < entityViews.count; ++i)
        {
            ...
            if ((viewFlags & cullingMask) == cullingMask)
            {
                ...
                if (ignoreCulling || Functions::IntersectAABB(cullingBounds, entityBounds))
                {
                    ...
                    isVisible |= (uint32_t)(rn[0] && rp[1] && rp[2] && rp[3] && entityBounds.max.x > cullingBoundsCenter.x) << PK_CUBE_FACE_RIGHT;
                    isVisible |= (uint32_t)(rp[0] && rn[1] && rn[2] && rn[3] && entityBounds.min.x < cullingBoundsCenter.x) << PK_CUBE_FACE_LEFT;
                    isVisible |= (uint32_t)(rp[0] && rp[1] && rp[4] && rn[5] && entityBounds.max.y > cullingBoundsCenter.y) << PK_CUBE_FACE_UP;
                    isVisible |= (uint32_t)(rn[0] && rn[1] && rn[4] && rp[5] && entityBounds.min.y < cullingBoundsCenter.y) << PK_CUBE_FACE_DOWN;
                    isVisible |= (uint32_t)(rp[2] && rn[3] && rp[4] && rp[5] && entityBounds.max.z > cullingBoundsCenter.z) << PK_CUBE_FACE_FRONT;
                    isVisible |= (uint32_t)(rn[2] && rp[3] && rn[4] && rn[5] && entityBounds.min.z < cullingBoundsCenter.z) << PK_CUBE_FACE_BACK;
    ...
    void EngineEntityCull::Step(RequestEntityCullCascades* request)
    {
        ...
        {
            cullingCascadePlanes[i] = Functions::ExtractFrustrumPlanes(request->cascades[i], true);
            ...
            if ((viewFlags & cullingMask) == cullingMask)
            {
                ...
                if (isVisible != 0u)
                {
                    ...
                    for (auto j = 0u; j < cullingCascadeCount; ++j)
                    {
                        if ((isVisible & (1 << j)) != 0u)
                        {
                            auto minDistLocal = Functions::PlaneMinDistanceToAABB(cullingCascadePlanes[j].near, entityBounds);
                            cullingMinDepth = glm::min(cullingMinDepth, minDistLocal);
                            // Need to do another loop to calculate this based on min-max.
                            m_synchronousResults.Add(entityId, Functions::PackHalf(minDistLocal), j);
                        }
                    }
                }
            }
        ...
        for (auto i = 0u; i < m_synchronousResults.count; ++i)
        {
            ...
            auto nearOffset = Functions::UnPackHalf(info.depth);
